...
export default function Expenses() {
  const { user } = useAuth();
  const isSuperAdmin = user?.role === 'superadmin' || user?.role === 'owner' || user?.role === 'branch_admin';
  ...
  const updateStatus = async (item, newStatus) => {
    try {
      await financeApi.expenses.updateStatus(item.id, newStatus);
      toast({ title: `Marked as ${newStatus}`, status: 'success' });
      fetchData();
    } catch (e) {
      toast({ title: 'Update failed', description: e?.response?.data?.message || e.message, status: 'error' });
    }
  };
                ...
                {loading ? (
                  ...
                  <Tr key={r.id} _hover={{ bg: rowHoverBg }}>
                    ...
                    <Td>
                      ...
                      {isSuperAdmin && <IconButton aria-label='Edit' icon={<MdEdit />} size='sm' variant='ghost' onClick={() => { setForm({ ...r, date: r.date?.slice(0, 10) }); editDisc.onOpen(); }} />}
                      {isSuperAdmin && <IconButton aria-label='Delete' icon={<MdDelete />} size='sm' variant='ghost' colorScheme='red' onClick={() => handleDelete(r.id)} />}

                      {isSuperAdmin && r.status === 'Pending' && <Button size='xs' ml={1} variant='outline' colorScheme='purple' onClick={() => updateStatus(r, 'Approved')}>Approve</Button>}
                      {isSuperAdmin && r.status === 'Pending' && <Button size='xs' ml={1} variant='outline' colorScheme='red' onClick={() => updateStatus(r, 'Rejected')}>Reject</Button>}
                      {isSuperAdmin && r.status === 'Approved' && <Button size='xs' ml={1} colorScheme='green' onClick={() => updateStatus(r, 'Paid')}>Pay</Button>}
                    </Td>
                  </Tr>
                ))}
              </Tbody>
        ...
        <ModalContent>
          ...
          <ModalBody pb={2}>
            ...
            <FormControl mb={3}>
              ...
              <Select value={form.status} onChange={(e) => setForm(f => ({ ...f, status: e.target.value }))} isDisabled={!isSuperAdmin}>
                <option value='Pending'>Pending</option>
                <option value='Approved'>Approved</option>
                <option value='Paid'>Paid</option>
                <option value='Rejected'>Rejected</option>
