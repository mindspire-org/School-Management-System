...
export default function Payroll() {
  ...
  const netSalaryBg = useColorModeValue('blue.50', 'blue.900');
  ...
  const { user } = useAuth();
  const isAdmin = user?.role === 'admin' || user?.role === 'owner' || user?.role === 'branch_admin';
          ...
          <Table variant='simple' size='sm'>
            ...
            <Tbody>
              {filtered.length === 0 ? (
                ...
                <Tr key={`${r.role}-${r.id}`} _hover={{ bg: rowHoverBg }}>
      ...
      <Modal isOpen={createDisc.isOpen} onClose={createDisc.onClose} size='lg'>
        ...
        <ModalContent>
          ...
          <ModalBody>
            ...
            <Box mt={4} p={3} bg={netSalaryBg} borderRadius='md'>
