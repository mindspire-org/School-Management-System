...
import { MdAdminPanelSettings, MdGroup, MdSecurity, MdFileDownload, MdAdd, MdRefresh, MdSearch, MdDelete } from 'react-icons/md';
...
export default function RoleManagement() {
  ...
  const buildModuleDefs = (roleId) => {
    try {
      ...
      let routes = [];
      let layout = null;
      if (r === 'admin' || r === 'owner' || r === 'branch_admin') {
        routes = getSMSRoutes();
        layout = '/admin';
      } else if (r === 'teacher') {
  ...
  return (
    <Box pt={{ base: '130px', md: '80px', xl: '80px' }}>
                ...
                <Tr key={r.id} _hover={{ bg: rowHoverBg }}>
                  ...
                  <Td>
                    <ButtonGroup size='sm' isAttached>
                      <Button variant='outline' onClick={() => { setSelected(r); editDisc.onOpen(); }}>Edit</Button>
                      <Tooltip label={r.id === 'owner' ? 'Owner role cannot be deleted' : r.users > 0 ? `Reassign ${r.users} user(s) first` : ''} hasArrow>
                        <Button
                          leftIcon={<MdDelete />}
                          colorScheme='red'
                          variant='outline'
                          isDisabled={r.id === 'owner' || r.users > 0}
                          onClick={() => { setRoleToDelete(r); deleteDisc.onOpen(); }}
                        >
                          Delete
                        </Button>
                      </Tooltip>
        ...
        <Flex justify="space-between" align="center" mb={4} direction={{ base: 'column', md: 'row' }} gap={3}>
          ...
          <Flex gap={3} align='center'>
            <Select maxW='220px' value={selectedRole} onChange={(e) => setSelectedRole(e.target.value)}>
              {roles.filter((r) => ['owner', 'admin', 'branch_admin', 'teacher', 'student', 'driver'].includes(r.id)).map(r => (
      ...
      <Modal isOpen={deleteDisc.isOpen} onClose={deleteDisc.onClose} isCentered>
        <ModalOverlay />
        <ModalContent>
          ...
          <ModalBody>
            {roleToDelete && (
              <>
                <Text mb={3}>Are you sure you want to delete the <strong>{roleToDelete.name}</strong> role?</Text>
                <Text color={textColorSecondary} fontSize='sm'>
                  This will deactivate the role and remove all its permissions and module access.
                  {roleToDelete.users > 0 && ` There are ${roleToDelete.users} user(s) assigned to this role.`}
                </Text>
              </>
            )}
          </ModalBody>
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  toast({ title: 'Role deleted', description: `${roleToDelete.name} has been deactivated.`, status: 'success', duration: 3000, isClosable: true });
                  const res = await rbacApi.getRoles();
                  setRoles(Array.isArray(res?.items) ? res.items : []);
                  deleteDisc.onClose();
                } catch (e) {
                  toast({ title: 'Delete failed', description: e?.response?.data?.message || 'Could not delete role.', status: 'error', duration: 4000, isClosable: true });
                }
              }}
            >
              Delete
